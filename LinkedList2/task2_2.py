...
# Рефлексия по 2.12.
# Альтернативным решением может быть проход по массиву с запоминанием минимального значения и добавлением его в новый LinkedList. Необходимо итерироваться по LinkedList n раз, и на каждой итерации находить минимальную node, после чего добавлять ее в конец нового LinkedList.
# Асимптотическая оценка O(n^2)
# Пространственная оценка O(n) - т.к. мы создаем дополнительное хранилище (результирующий список) длинной n элементов.
  
# Задание 2.13.
# Склеивание двух LinkedList.
# Асимптотическая оценка O(n^2) - в данном случае сортировка является узким горлышком.
# Пространственная оценка O(n) - необходимо создать новый LinkedList длинной n элементов.
...
# Рефлексия 2.13.
# Альтернативным решением может являться использование другого алгоритма сортировки, работающего быстрее O(n^2).
